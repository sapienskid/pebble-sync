...
const DEFAULT_SETTINGS = {
  // --- General Settings ---
  apiBaseUrl: 'https://pebble.sp03201122.workers.dev',
  apiKey: '',
...
class PebbleSyncPlugin extends Plugin {
  ...
  async importNow(force = false) {
    ...
    if (!s.apiBaseUrl || !s.apiKey) {
      new Notice('Pebble Sync: API URL and Key are required.');
      return;
    }
    ...
    try {
      const resp = await fetch(`${s.apiBaseUrl.replace(/\/+$/, '')}/api/sync/fetch`, {
        method: 'GET',
...
class PebbleSyncSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(containerEl).setName('API Base URL').addText(t => t.setPlaceholder('https://pebble...').setValue(this.plugin.settings.apiBaseUrl).onChange(async v => { this.plugin.settings.apiBaseUrl = v.trim(); await this.plugin.saveSettings(); }));
    new Setting(containerEl).setName('API Key').addText(t => { t.setPlaceholder('Your API key').setValue(this.plugin.settings.apiKey).onChange(async v => { this.plugin.settings.apiKey = v.trim(); await this.plugin.saveSettings(); }); t.inputEl.type = 'password'; });
    ...
    if (this.plugin.settings.atomicNotesEnabled) {
      ...
      new Setting(containerEl).setName('Atomic note template').setDesc(
        'Available variables: {{content}}, {{date}}, {{time}}, {{fullDateTime}}, {{tags}} (comma-separated string).'
      ).addTextArea(text => {
